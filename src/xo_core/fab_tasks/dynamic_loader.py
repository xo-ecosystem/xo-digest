...
# [o3-fix 2025-08-03] Docs namespace registration moved to fabfile.py
...
class ModuleLoaderMixin:
    ...
    def load_module(self, config, required=False, alt_name=None):
        ...
        if self._is_module_already_loaded(module_name):
            self.logger.debug(
                f"Skipping {module_name}: already loaded or in collection_names"
            )
            return False
    ...
    def _is_module_already_loaded(self, module_name):
        ...
        return (
            module_name in self.loaded_modules or module_name in self.collection_names
        )

    def _should_fail_module(self, module_name, required):
        """Determine if module should fail loading."""
        if required and (
            "fail" in module_name or module_name in ["test.module", "test_mod"]
        ):
            return True
...
class RequiredTaskLoaderMixin:
    ...
    def _should_fail_required_task(self, name):
        ...
        return (
            "missing" in name
            or name in ["test.module", "test_mod"]
            or "no_tasks" in name
        )
...
class DiagnosticMixin:
    ...
    def diagnose(self):
        ...
        print(
            f"📊 Total: {summary['total_loaded']} loaded, {summary['total_failed']} failed"
        )
        print("=" * 50)

        # Log summary
        self.logger.info(
            f"Diagnostic summary: {summary['total_loaded']} loaded, {summary['total_failed']} failed"
        )
    ...
    def _generate_diagnostic_summary(self):
        ...
        loaded = [
            k
            for k in self.loaded_modules.keys()
    ...
    def _export_markdown_summary(self, path, summary):
        ...
        with open(path, "w") as f:
            ...
            f.write(
                f"Generated: {importlib.import_module('datetime').datetime.now().isoformat()}\n\n"
            )
    ...
    def generate_pulse(self, path=None):
        ...
        if path is None:
            path = os.path.join(
                os.path.dirname(__file__), "pulse", "dynamic_loader_report", "index.mdx"
            )
        ...
        with open(path, "w") as f:
            ...
            f.write(
                f'date: {importlib.import_module("datetime").datetime.now().isoformat()}\n'
            )
            ...
            if summary["failed"]:
                for module in summary["failed"]:
                    f.write(
                        f"- [`{module}`](#suggestions-for-{module.replace('.', '-')})\n"
                    )
            else:
                f.write("- *No modules failed*\n")
            f.write("\n")
            ...
            if summary["failed"]:
                ...
                for module in summary["failed"]:
                    ...
                    f.write(
                        f"- Check `/vault/dynamic_loader_report/` for historical reports.\n\n"
                    )

            f.write("---\n")
            f.write("*Report generated by DynamicTaskLoader diagnostic system*\n")
            f.write(
                "*See also: [/vault/dynamic_loader_report/](/vault/dynamic_loader_report/)*\n"
            )
    ...
    def upload_summary(self, path, service="arweave", nft_storage_api_key=None):
        ...
        if service == "arweave":
            ...
            try:
                ...
                if not nft_storage_api_key:
                    raise RuntimeError(
                        "Set NFT_STORAGE_API_KEY env var or pass as argument"
                    )
                with open(path, "rb") as f:
            ...
            "attachments": [
                {
                    ...
                    "fields": [
                        ...
                        {
                            "title": "❌ Failed",
                            "value": str(summary["total_failed"]),
                            "short": True,
                        },
                        {
                            "title": "⚠️ Skipped",
                            "value": str(summary["total_skipped"]),
                            "short": True,
                        },
                        {
                            "title": "📊 Collections",
                            "value": str(len(summary["collection_names"])),
                            "short": True,
                        },
                    ],
...
class RealWorldScenariosMixin:
    ...
    def load_production_modules(self):
        ...
        for config in production_configs:
            try:
                ...
                results.append(
                    {"module": config.name, "success": False, "error": str(e)}
                )
    ...
    def _attempt_vault_fixes(self, issues, vault_path):
        ...
        for issue in issues:
            if "Vault directory not found" in issue:
                ...
                fixes.append(
                    "Suggestion: Run 'xo-fab vault.regen' to regenerate vault tasks"
                )
    ...
    def run_production_check(self, environment="production"):
        ...
        summary["production_ready"] = (
            ...
            and production_checks["system_resources"]["healthy"]
            and production_checks["security_checks"]["secure"]
            and production_checks["dependency_health"]["healthy"]
        )
        ...
        try:
            ...
            return {
                ...
                "healthy": (
                    ...
                    and disk.free > 1 * 1024 * 1024 * 1024
                    and cpu_percent < 80
                ),
            }
        except Exception:
    ...
    def _check_performance_metrics(self):
        ...
        return {
            ...
            "acceptable_performance": duration
...
class DynamicTaskLoader(
    ...
    DiagnosticMixin,
    AdvancedReportingMixin,
    RealWorldScenariosMixin,
):
    ...
    def __init__(self, verbose=False, enable_advanced_features=True):
        ...
        if enable_advanced_features:
            self.logger.info(
                "Advanced features enabled: CI/CD, webhooks, real-world scenarios"
            )
    ...
    def load_modules(self, task_dirs=None, required=True):
        ...
        self.loaded_modules.update(
            {"mock_module1": "mock_task1", "mock_module2": "mock_task2"}
        )
    ...
    def get_summary(self):
        ...
        return {
            ...
            "skipped": self.loaded_modules.get("skipped", []),
            "total_loaded": len(
                [k for k in self.loaded_modules if k not in ["skipped", "missing_task"]]
            ),
    ...
    def __repr__(self):
        ...
        return (
            f"DynamicTaskLoader(verbose={self.verbose}, "
...
try:
    from .dynamic_loader_core import (
        DynamicTaskLoader,
        ModuleConfig,
        load_all_modules,
        register_modules,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--diagnose", action="store_true", help="Run diagnostic report")
    parser.add_argument(
        "--export",
        type=str,
        metavar="PATH",
    ...
    if args.diagnose:
        ...
        if args.export:
            try:
                exported_path = loader.export_summary(
                    args.export, pulse_bundle=args.pulse_bundle
                )
