...
try:
    from xo_core.vault.webhook import format_pin_summary
    from xo_core.vault.digest import generate_pin_digest
    from xo_core.vault.inbox import send_to_xo_inbox

    print("✅ Successfully imported vault modules (xo_core.vault.*)")
    ...
    try:
        with open(file_path, "rb") as f:
            ...
            response = requests.post(
                PIN_CONFIG["ipfs"]["api_url"], headers=headers, files=files
            )
        ...
        if response.status_code == 200:
            ...
            log_pin_activity(
                f"IPFS pin failed: {file_path.name} -> {response.status_code}"
            )
            return None
    except Exception as e:
        ...
        if result.returncode == 0:
            ...
            for line in result.stdout.split("\n"):
                if "TX ID:" in line:
                    ...
                    log_pin_activity(
                        f"Arweave pin successful: {file_path.name} -> {txid}"
                    )

                    # Save Arweave TXID marker
                    arweave_marker = file_path.with_suffix(
                        file_path.suffix + ".arweave_tx"
                    )
            ...
            log_pin_activity(
                f"Arweave pin successful: {file_path.name} -> (no TXID in output)"
            )
            return "uploaded"
        else:
...
def save_pin_status(healthy: int, broken: int, missing: int) -> None:
    ...
    status_data = {
        ...
        "health_percentage": (
            round((healthy / (healthy + broken + missing)) * 100, 1)
            if (healthy + broken + missing) > 0
            else 0
        ),
    }
...
def check_pin_health(
    c, slug: Optional[str] = None, repair: bool = False
) -> Tuple[int, int, int]:
            ...
            if pinned_at:
                try:
                    ...
                    if datetime.now() - pin_date > timedelta(days=30):
                        print_log(
                            f"  ⚠️ Pin is old ({pin_date.strftime('%Y-%m-%d')})", "WARN"
                        )
                        if repair:
...
def pin_status(c, repair=False, slug=None, verbose=False):
    ...
    if total > 0:
        ...
        print_log(
            f"📊 Overall Health: {health_percentage}% ({healthy}/{total})", "INFO"
        )
...
def pin_all(
    ...
    regular_files = [
        ...
        if not any(
            f.name.endswith(marker) for marker in PIN_CONFIG["file_types"]["markers"]
        )
    ]
    ...
    print_log(
        f"📦 Found {len(regular_files)} regular files and {len(txid_files)} .txid files",
        "INFO",
    )
        ...
        for file_path in sorted(txid_files):
            ...
            try:
                ...
                pin_meta = {
                    ...
                    "pinned_at": datetime.now().isoformat(),
                    "ipfs_gateway": (
                        PIN_CONFIG["ipfs"]["gateway"].format(cid=ipfs_cid)
                        if ipfs_cid
                        else None
                    ),
                    "arweave_link": (
                        PIN_CONFIG["arweave"]["gateway"].format(txid=arweave_txid)
                        if arweave_txid
                        else None
                    ),
                }
    ...
    print_log(
        f"  📦 Regular files: {pinned} pinned, {skipped} skipped, {failed} failed",
        "INFO",
    )
    if txid_files:
        print_log(
            f"  🔗 TXID files: {txid_pinned} pinned, {txid_skipped} skipped, {txid_failed} failed",
            "INFO",
        )
    ...
    if txid_files:
        ...
        print_log(
            f"  📊 Total: {total_pinned} pinned ({file_types_str}), {total_skipped} skipped, {total_failed} failed",
            "INFO",
        )
    else:
        print_log(
            f"  📊 Total: {total_pinned} pinned, {total_skipped} skipped, {total_failed} failed",
            "INFO",
        )
    ...
    if check_health:
        ...
        if healthy > 0 and broken == 0 and missing == 0:
            ...
            print_log(
                f"⚠️ Health issues found: {broken} broken, {missing} missing", "WARN"
            )
    ...
    print_log(
        f"✅ Total Pinned: {total_pinned} ({pinned} regular, {txid_pinned} .txid)",
        "INFO",
    )
    if check_health:
        print_log(
            (
                "🛡️ Verified: All pins healthy"
                if total_failed == 0
                else f"⚠️ Issues: {total_failed} failed"
            ),
            "INFO",
        )
