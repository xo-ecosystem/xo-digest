...
def validate_email_deliverability(
    ...
    try:
        try:
            ...
            if (
                len(mtas) == 0
            ):  # null MX only, if there were no MX records originally a NoAnswer exception would have occurred
                raise EmailUndeliverableError(
                    f"The domain name {domain_i18n} does not accept email."
                )
                ...
                for rec in response:
                    ...
                    if value.startswith(b"v=spf1 "):
                        if value == b"v=spf1 -all":
                            raise EmailUndeliverableError(
                                f"The domain name {domain_i18n} does not send email."
                            )
            except dns.resolver.NoAnswer:
        ...
        # for the domain name.
        raise EmailUndeliverableError(
            f"The domain name {domain_i18n} does not exist."
        ) from e
