...
from Crypto.Math.Primality import test_probable_prime, COMPOSITE, PROBABLY_PRIME
...
class DsaKey:
    ...
    def __init__(self, key_dict):
        ...
        if not public_set.issubset(input_set):
            raise ValueError(
                "Some DSA components are missing = %s" % str(public_set - input_set)
            )
    ...
    def __eq__(self, other):
        ...
        for comp in self._keydata:
            result = result and (
                getattr(self._key, comp, None) == getattr(other._key, comp, None)
            )
        return result
    ...
    def __repr__(self):
        ...
        return "<{} @0x{:x} {}>".format(
            self.__class__.__name__, id(self), ",".join(attrs)
        )
...
def generate(bits, randfunc=None, domain=None):
    ...
    if L != bits:
        raise ValueError(
            "Mismatch between size of modulus (%d)"
            " and 'bits' parameter (%d)" % (L, bits)
        )

    if (L, N) not in [(1024, 160), (2048, 224), (2048, 256), (3072, 256)]:
        raise ValueError(
            "Lengths of p and q (%d, %d) are not compatible" "to FIPS 186-3" % (L, N)
        )
