...
for idx, tv in enumerate(test_vectors_verify):
    ...
    def ecdh_test(
        self, public_key=public_key, private_key=private_key, exp_response=exp_response
    ):
        z = key_agreement(
            static_pub=public_key, static_priv=private_key, kdf=lambda x: x
        )
...
class TestVectorsECDHWycheproof(unittest.TestCase):
    ...
    def runTest(self):
        for tv in self.tv:
            self.desc = "Wycheproof ECDH Verify Test #%d (%s, %s)" % (
                tv.id,
                tv.comment,
                tv.filename,
            )
...
class X25519_Tests(unittest.TestCase):
    ...
    def test_rfc7748_1(self):
        ...
        for tv1, tv2, tv3 in tvs:
            ...
            result = key_agreement(
                static_pub=pub_key, static_priv=priv_key, kdf=lambda x: x
            )
    ...
    def test_rfc7748_3(self):
        ...
        result1 = key_agreement(
            static_pub=alice_pub_key, static_priv=bob_priv_key, kdf=lambda x: x
        )
        result2 = key_agreement(
            static_pub=bob_pub_key, static_priv=alice_priv_key, kdf=lambda x: x
        )
...
class TestVectorsX25519Wycheproof(unittest.TestCase):
    ...
    def runTest(self):
        for tv in self.tv:
            self.desc = "Wycheproof XECDH Verify Test #%d (%s, %s)" % (
                tv.id,
                tv.comment,
                tv.filename,
            )
            ...
            try:
                ...
                if tv.valid:
                    ...
                    if len(tv.public["x"]) == 75:
                        assert (
                            "LowOrderPublic" in tv.flags
                            or "NonCanonicalPublic" in tv.flags
                        )
                        assert "Invalid Curve448" in str(e)
                    else:
                        assert "Incorrect Curve448" in str(e)
