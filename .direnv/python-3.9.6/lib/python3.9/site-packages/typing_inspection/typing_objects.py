...
def _compile_identity_check_function(
    member: LiteralString, function_name: LiteralString
) -> FunctionType:
...
def _compile_isinstance_check_function(
    member: LiteralString, function_name: LiteralString
) -> FunctionType:
    ...
    if in_typing and in_typing_extensions:
        if getattr(typing, member) is getattr(typing_extensions, member):
            ...
            check_code = (
                f"isinstance(obj, (typing.{member}, typing_extensions.{member}))"
            )
    elif in_typing and not in_typing_extensions:
        check_code = f"isinstance(obj, typing.{member})"
    elif not in_typing and in_typing_extensions:
...
is_paramspecargs = _compile_isinstance_check_function(
    "ParamSpecArgs", "is_paramspecargs"
)
...
is_paramspeckwargs = _compile_isinstance_check_function(
    "ParamSpecKwargs", "is_paramspeckwargs"
)
...
_is_typealiastype_inner = _compile_isinstance_check_function(
    "TypeAliasType", "_is_typealiastype_inner"
)
...
if _IS_PY310:
    ...
    is_typealiastype = _compile_isinstance_check_function(
        "TypeAliasType", "is_typealiastype"
    )
