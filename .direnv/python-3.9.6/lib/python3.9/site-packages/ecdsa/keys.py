    ...
    if not allow_truncate:
        if len(digest) > curve.baselen:
            raise BadDigestError(
                "this curve ({}) is too short "
                "for the length of your digest ({})".format(curve.name, 8 * len(digest))
            )
    else:
...
class VerifyingKey:
    ...
    def __repr__(self):
        ...
        if self.default_hashfunc:
            ...
            hash_name = "None"
        return "VerifyingKey.from_string({!r}, {!r}, {})".format(
    ...
    def from_der(
        ...
        if not oid_pk == oid_ecPublicKey:
            raise der.UnexpectedDER(
                "Unexpected object identifier in DER " "encoding: {!r}".format(oid_pk)
            )
...
class SigningKey:
    ...
    def from_string(cls, string, curve=NIST192p, hashfunc=sha1):
        ...
        if len(string) != curve.baselen:
            raise MalformedPointError(
                "Invalid length of private key, received {}, "
                "expected {}".format(len(string), curve.baselen)
            )
        if isinstance(curve.curve, CurveEdTw):
            self = cls(_error__please_use_generate=True)
            self.curve = curve
    ...
    def from_der(cls, string, hashfunc=sha1, valid_curve_encodings=None):
        ...
        if der.is_sequence(s):
            ...
            if algorithm_oid not in (oid_ecPublicKey, oid_ecDH, oid_ecMQV):
                raise der.UnexpectedDER(
                    f"unexpected algorithm identifier '{algorithm_oid}'"
                )
