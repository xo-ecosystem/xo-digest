...
class PRNG:
    ...
    def block_generator(self, seed):
        counter = 0
        while True:
            yield from sha256(("prng-%d-%s" % (counter, seed)).encode()).digest()
            counter += 1
...
def sigdecode_string(signature, order):
    ...
    if not len(signature) == 2 * l:
        raise MalformedSignature(
            "Invalid length of signature, expected {} bytes long, "
            "provided string is {} bytes long".format(2 * l, len(signature))
        )
...
def sigdecode_strings(rs_strings, order):
    ...
    if not len(r_str) == l:
        raise MalformedSignature(
            ...
            "expected {} bytes long, provided string is {} "
