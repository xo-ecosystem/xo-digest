...
class ECDSA(unittest.TestCase):
    ...
    def test_deterministic(self):
        ...
        priv = SigningKey.from_secret_exponent(secexp, SECP256k1, hashlib.sha256)
        pub = priv.get_verifying_key()
    ...
    def test_pubkey_strings(self):
        ...
        self.assertRaises(der.UnexpectedDER, VerifyingKey.from_der, pub1_der + b"junk")
    ...
    def test_sigencode_string_canonize_no_change(self):
        ...
        new_r, new_s = sigdecode_string(sigencode_string_canonize(r, s, order), order)
    ...
    def test_sigencode_strings_canonize_no_change(self):
        ...
        new_r, new_s = sigdecode_strings(sigencode_strings_canonize(r, s, order), order)
    ...
    def test_sigencode_der_canonize_no_change(self):
        ...
        new_r, new_s = sigdecode_der(sigencode_der_canonize(r, s, order), order)
...
for curve in curves:
    for enc in ["raw", "uncompressed", "compressed", "hybrid"]:
        params.append(pytest.param(curve, enc, id=f"{curve.name}-{enc}"))
...
class OpenSSL(unittest.TestCase):
    ...
    try:
        if "-rawin" in run_openssl("pkeyutl -help"):
            OPENSSL_SUPPORTED_TYPES = {  # pragma: no branch
                c.lower()
    ...
    def do_eddsa_test_from_openssl(self, curve):
        ...
        run_openssl(f"genpkey -algorithm {curvename} -outform PEM -out t/privkey.pem")
        run_openssl("pkey -outform PEM -pubout -in t/privkey.pem -out t/pubkey.pem")
...
class Util(unittest.TestCase):
    ...
    def OFF_test_prove_uniformity(self):  # pragma: no cover
        order = 2**8 - 2
        counts = {i: 0 for i in range(1, order)}
