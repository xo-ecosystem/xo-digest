...
def encode_implicit(tag, value, cls="context-specific"):
    ...
    return bytes((tag_class + tag,)) + encode_length(len(value)) + value
...
def remove_implicit(string, exp_class="context-specific"):
    ...
    if (s0 & tag_mask) != tag_class:
        raise UnexpectedDER(f"wanted class {exp_class}, got 0x{s0:02x} tag")
    if s0 & 0b00100000 != 0:
        raise UnexpectedDER(f"wanted type primitive, got 0x{s0:02x} tag")
