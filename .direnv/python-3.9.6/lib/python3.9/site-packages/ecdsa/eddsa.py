...
class PublicKey:
    ...
    def __init__(self, generator, public_key, public_point=None):
        ...
        if len(public_key) != self.baselen:
            raise ValueError(
                "Incorrect size of the public key, expected: {} bytes".format(
                    self.baselen
                )
            )
        if public_point:
    ...
    def verify(self, data, signature):
        ...
        if len(signature) != 2 * self.baselen:
            raise ValueError(
                f"Invalid signature length, expected: {2 * self.baselen} bytes"
            )
...
class PrivateKey:
    ...
    def __init__(self, generator, private_key):
        ...
        if len(private_key) != self.baselen:
            raise ValueError(
                f"Incorrect size of private key, expected: {self.baselen} bytes"
            )
