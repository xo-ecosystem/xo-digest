...
class TestJacobi(unittest.TestCase):
    ...
    @given(
        st.integers(min_value=0, max_value=int(generator_brainpoolp160r1.order() - 1))
    )
    ...
    @settings(**SLOW_SETTINGS)
    @given(
        st.integers(min_value=1, max_value=int(generator_brainpoolp160r1.order() - 1)),
        st.integers(min_value=1, max_value=int(generator_brainpoolp160r1.order() - 1)),
    )
    @example(3, 3)
    ...
    @pytest.mark.slow
    @pytest.mark.skipif(
        platform.system() == "Windows" or platform.python_implementation() == "PyPy",
