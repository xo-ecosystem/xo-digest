...
class CryptographyRSAKey(Key):
    ...
    def __init__(self, key, algorithm, cryptography_backend=default_backend):
        ...
        if (hasattr(key, "public_bytes") and hasattr(key, "public_numbers")) or hasattr(
            key, "private_bytes"
        ):
            self.prepared_key = key
            return
...
class CryptographyAESKey(Key):
    ...
    def encrypt(self, plain_text, aad=None):
        ...
        try:
            iv_byte_length = self.IV_BYTE_LENGTH_MODE_MAP.get(
                self._mode.name, algorithms.AES.block_size
            )
