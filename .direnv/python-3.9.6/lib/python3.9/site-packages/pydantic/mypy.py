...
class PydanticPlugin(Plugin):
    ...
    def _pydantic_model_class_maker_callback(self, ctx: ClassDefContext) -> None:
        transformer = PydanticModelTransformer(
            ctx.cls, ctx.reason, ctx.api, self.plugin_config
        )
...
class PydanticModelField:
    ...
    def to_argument(
        ...
        if typed or strict:
            type_annotation = self.expand_type(
                current_info, api, include_root_type=True
            )
        else:
    ...
    def expand_type(
        ...
        if self.type is not None and self.info.self_type is not None:
            ...
            with state.strict_optional_set(api.options.strict_optional):
                ...
                expanded_type = expand_type(
                    self.type, {self.info.self_type.id: filled_with_typevars}
                )
                if (
                    include_root_type
                    and isinstance(expanded_type, Instance)
                    and is_root_model(expanded_type.type)
                ):
                    ...
                    expanded_root_type = expand_type_by_instance(
                        root_type, expanded_type
                    )
...
class PydanticModelTransformer:
    ...
    def transform(self) -> bool:
        ...
        * stores the fields, config, and if the class is settings in the mypy metadata for access by subclasses
        ...
        info.metadata[METADATA_KEY] = {
            ...
            "class_vars": {
                class_var.name: class_var.serialize() for class_var in class_vars
            },
                ...
                if isinstance(stmt.rvalue, CallExpr):  # calls to `dict` or `ConfigDict`
                    for arg_name, arg in zip(stmt.rvalue.arg_names, stmt.rvalue.args):
                        if arg_name is None:
                            continue
                        config.update(
                            self.get_config_update(arg_name, arg, lax_extra=True)
                        )
                    ...
                    for key_expr, value_expr in stmt.rvalue.items:
                        if not isinstance(key_expr, StrExpr):
                            continue
                        config.update(
                            self.get_config_update(key_expr.value, value_expr)
                        )
    ...
    def collect_fields_and_class_vars(
        ...
        for info in reversed(
            cls.info.mro[1:-1]
        ):  # 0 is the current class, -2 is BaseModel, -1 is object
        ...
        for stmt in self._get_assignment_statements_from_block(cls.defs):
            maybe_field = self.collect_field_or_class_var_from_stmt(
                stmt, model_config, found_class_vars
            )
            ...
            assert isinstance(
                lhs, NameExpr
            )  # collect_field_or_class_var_from_stmt guarantees this
    ...
    def collect_field_or_class_var_from_stmt(  # noqa C901
        ...
        class_vars: dict[str, PydanticModelClassVar],
        ...
        lhs = stmt.lvalues[0]
        if (
            not isinstance(lhs, NameExpr)
            or not _fields.is_valid_field_name(lhs.name)
    ...
    def add_initializer(
        ...
        use_alias = (
            not (config.validate_by_name or config.populate_by_name)
            and config.validate_by_alias is not False
        )
        requires_dynamic_aliases = bool(
            config.has_alias_generator and not config.validate_by_name
        )
        ...
        if is_settings:
            ...
            if "__init__" in base_settings_node.names:
                ...
                if (
                    base_settings_init_node is not None
                    and base_settings_init_node.type is not None
                ):
                    ...
                    for arg_idx, arg_name in enumerate(func_type.arg_names):
                        if (
                            arg_name is None
                            or arg_name.startswith("__")
                            or not arg_name.startswith("_")
                        ):
                            continue
                        analyzed_variable_type = self._api.anal_type(
                            func_type.arg_types[arg_idx]
                        )
                        if (
                            analyzed_variable_type is not None
                            and arg_name == "_cli_settings_source"
                        ):
                        ...
                        args.append(
                            Argument(variable, analyzed_variable_type, None, ARG_OPT)
                        )
    ...
    def add_model_construct_method(
        ...
        """
        set_str = self._api.named_type(
            f"{BUILTINS_NAME}.set", [self._api.named_type(f"{BUILTINS_NAME}.str")]
        )
        ...
        args = (
            args + [fields_set_argument]
            if is_root_model
            else [fields_set_argument] + args
        )
    ...
    def get_config_update(
        self, name: str, arg: Expression, lax_extra: bool = False
        ...
        if isinstance(arg, NameExpr) and arg.fullname in (
            "builtins.True",
            "builtins.False",
        ):
        ...
        if (
            ...
            for arg, name in zip(expr.args, expr.arg_names):
                ...
                if name == "default_factory":
                    return not (
                        isinstance(arg, NameExpr) and arg.fullname == "builtins.None"
                    )
            return False
...
ERROR_EXTRA_FIELD_ROOT_MODEL = ErrorCode(
    "pydantic-field", "Extra field on RootModel subclass", "Pydantic"
)
...
def error_from_attributes(
    ...
    api.fail(
        f'"{model_name}" does not have from_attributes=True', context, code=ERROR_ORM
    )
