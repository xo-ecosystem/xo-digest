...
def apply_known_metadata(
    annotation: Any, schema: CoreSchema
) -> CoreSchema | None:  # noqa: C901
        ...
        if constraint in chain_schema_constraints:
            ...
            chain_schema_steps.append(
                cs.no_info_wrap_validator_function(
                    _apply_constraint_with_incompatibility_info,
                    cs.str_schema(**{constraint: value}),
                )
            )
            ...
            schema = cs.no_info_after_validator_function(
                partial(NUMERIC_VALIDATOR_LOOKUP[constraint], **{constraint: value}),
                schema,
            )
            metadata = schema.get("metadata", {})
            if (
                existing_json_schema_updates := metadata.get("pydantic_js_updates")
            ) is not None:
            ...
            raise RuntimeError(
                f"Unable to apply constraint '{constraint}' to schema of type '{schema_type}'"
            )

    for annotation in other_metadata:
        if (annotation_type := type(annotation)) in (
            at_to_constraint_map := _get_at_to_constraint_map()
        ):
            ...
            if validator is None:
                raise ValueError(f"Unknown constraint {constraint}")
            schema = cs.no_info_after_validator_function(
                partial(validator, {constraint: getattr(annotation, constraint)}),
                schema,
            )
            continue
        elif isinstance(annotation, (at.Predicate, at.Not)):
            predicate_name = (
                f"{annotation.func.__qualname__}"
...
def collect_known_metadata(
    ...
    for annotation in annotations:
        ...
        elif (annotation_type := type(annotation)) in (
            at_to_constraint_map := _get_at_to_constraint_map()
        ):
            constraint = at_to_constraint_map[annotation_type]
...
def check_metadata(
    metadata: dict[str, Any], allowed: Iterable[str], source_type: Any
) -> None:
