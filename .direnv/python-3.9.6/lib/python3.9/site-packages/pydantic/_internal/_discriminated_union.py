...
class _ApplyInferredDiscriminator:
    ...
    * Handling various edge cases such as 'definitions', 'default', 'nullable' schemas, and more
    ...
    def __init__(
        self, discriminator: str, definitions: dict[str, core_schema.CoreSchema]
    ):
    ...
    def _apply_to_root(self, schema: core_schema.CoreSchema) -> core_schema.CoreSchema:
        ...
        if (
            self._discriminator_alias is not None
            and self._discriminator_alias != self.discriminator
        ):
            ...
            discriminator: str | list[list[str | int]] = [
                [self.discriminator],
    ...
    def _handle_choice(self, choice: core_schema.CoreSchema) -> None:
        ...
        * Updating the _tagged_union_choices mapping that will ultimately be used to build the TaggedUnionSchema.
        ...
        if choice["type"] == "none":
            ...
            if choice["type"] == "tagged-union" and self._is_discriminator_shared(
                choice
            ):
                ...
                subchoices = [
                    x
                    for x in choice["choices"].values()
            ...
            inferred_discriminator_values = self._infer_discriminator_values_for_choice(
                choice, source_name=None
            )
    ...
    def _is_discriminator_shared(self, choice: core_schema.TaggedUnionSchema) -> bool:
        ...
        return inner_discriminator == self.discriminator or (
            ...
            and (
                self.discriminator in inner_discriminator
    ...
    def _infer_discriminator_values_for_choice(  # noqa C901
        ...
        if choice["type"] == "definitions":
            return self._infer_discriminator_values_for_choice(
                choice["schema"], source_name=source_name
            )
            ...
            return sorted(
                set(
                    self._infer_discriminator_values_for_choice(
                        choice["lax_schema"], source_name=None
                    )
                    + self._infer_discriminator_values_for_choice(
                        choice["strict_schema"], source_name=None
                    )
                )
            )
            ...
            for subchoice in subchoices:
                subchoice_values = self._infer_discriminator_values_for_choice(
                    subchoice, source_name=None
                )
            ...
            for subchoice in choice["choices"]:
                subchoice_schema = (
                    subchoice[0] if isinstance(subchoice, tuple) else subchoice
                )
                subchoice_values = self._infer_discriminator_values_for_choice(
                    subchoice_schema, source_name=None
                )
                values.extend(subchoice_values)
            return values
            ...
            self._should_be_nullable = True
            return self._infer_discriminator_values_for_choice(
                choice["schema"], source_name=None
            )

        elif choice["type"] == "model":
            return self._infer_discriminator_values_for_choice(
                choice["schema"], source_name=choice["cls"].__name__
            )
            ...
            return self._infer_discriminator_values_for_model_choice(
                choice, source_name=source_name
            )

        elif choice["type"] == "dataclass-args":
            return self._infer_discriminator_values_for_dataclass_choice(
                choice, source_name=source_name
            )

        elif choice["type"] == "typed-dict":
            return self._infer_discriminator_values_for_typed_dict_choice(
                choice, source_name=source_name
            )
            ...
            return self._infer_discriminator_values_for_choice(
                self.definitions[schema_ref], source_name=source_name
            )
        else:
            err_str = f'The core schema type {choice["type"]!r} is not a valid discriminated union variant.'
            if choice["type"] == "list":
    ...
    def _infer_discriminator_values_for_typed_dict_choice(
        ...
        if field is None:
            raise PydanticUserError(
                f"{source} needs a discriminator field for key {self.discriminator!r}",
                code="discriminator-no-field",
            )
    ...
    def _infer_discriminator_values_for_dataclass_choice(
        ...
        source = (
            "DataclassArgs" if source_name is None else f"Dataclass {source_name!r}"
        )
    ...
    def _infer_discriminator_values_for_field(
        self, field: CoreSchemaField, source: str
    ) -> list[str | int]:
        ...
        if not isinstance(alias, str):
            raise PydanticUserError(
                f"Alias {alias!r} is not supported in a discriminated union",
                code="discriminator-alias-type",
            )
        ...
        return self._infer_discriminator_values_for_inner_schema(
            field["schema"], source
        )
    ...
    def _infer_discriminator_values_for_inner_schema(
        ...
        if schema["type"] == "literal":
            ...
            for choice in schema["choices"]:
                ...
                choice_values = self._infer_discriminator_values_for_inner_schema(
                    choice_schema, source
                )
    ...
    def _set_unique_choice_for_values(
        self, choice: core_schema.CoreSchema, values: Sequence[str | int]
    ) -> None:
