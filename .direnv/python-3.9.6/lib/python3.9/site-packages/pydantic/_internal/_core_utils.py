...
def _clean_schema_for_pretty_print(
    obj: Any, strip_metadata: bool = True
) -> Any:  # pragma: no cover
    ...
    if isinstance(obj, Mapping):
        ...
        for k, v in obj.items():
            if k == "metadata" and strip_metadata:
                ...
                for meta_k, meta_v in v.items():
                    if meta_k in (
                        ...
                        new_metadata[meta_k] = _clean_schema_for_pretty_print(
                            meta_v, strip_metadata=strip_metadata
                        )
            ...
            # Remove some defaults:
                ...
                new_dct[k] = _clean_schema_for_pretty_print(
                    v, strip_metadata=strip_metadata
                )

        return new_dct
    elif isinstance(obj, Sequence) and not isinstance(obj, str):
        return [
            _clean_schema_for_pretty_print(v, strip_metadata=strip_metadata)
            for v in obj
        ]
    else:
        return obj
...
def pretty_print_core_schema(
    ...
    *,
    ...
    if (inspect.isclass(val) and issubclass(val, BaseModel)) or is_pydantic_dataclass(
        val
    ):
