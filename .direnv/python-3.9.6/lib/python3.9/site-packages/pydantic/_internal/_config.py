...
class ConfigWrapper:
    ...
    def core_config(self, title: str | None) -> core_schema.CoreConfig:
        ...
        if (
            config.get("validate_by_alias") is False
            and config.get("validate_by_name") is None
        ):
            config["validate_by_name"] = True

        if (not config.get("validate_by_alias", True)) and (
            not config.get("validate_by_name", False)
        ):
            raise PydanticUserError(
