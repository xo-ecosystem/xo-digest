...
_mode_to_validator: dict[
    FieldValidatorModes,
    type[BeforeValidator | AfterValidator | PlainValidator | WrapValidator],
] = {
...
class GenerateSchema:
    ...
    def _enum_schema(self, enum_type: type[Enum]) -> CoreSchema:
        ...
        if cases:
            ...
            default_missing = (
                getattr(enum_type._missing_, "__func__", None)
            ...
            def get_json_schema_no_cases(
                _, handler: GetJsonSchemaHandler
            ) -> JsonSchemaValue:
                json_schema = handler(
        ...
        return core_schema.lax_or_strict_schema(
            ...
            metadata={
                "pydantic_js_functions": [
                    lambda _1, _2: {
                        "type": "string",
                        "format": ip_type_json_schema_format[tp],
                    }
                ]
            },
        )
    ...
    def _path_schema(self, tp: Any, path_type: Any) -> CoreSchema:
        if tp is os.PathLike and (
            path_type not in {str, bytes} and not typing_objects.is_any(path_type)
        ):
            raise PydanticUserError(
        ...
        strict_inner_schema = (
            ...
            else core_schema.str_schema(strict=True)
        )
        lax_inner_schema = (
        ...
        def ser_path(
            path: Any, info: core_schema.SerializationInfo
        ) -> str | os.PathLike[Any]:
        ...
        instance_schema = core_schema.json_or_python_schema(
            json_schema=core_schema.no_info_after_validator_function(
                path_validator, lax_inner_schema
            ),
            python_schema=core_schema.is_instance_schema(tp),
        )
            ...
            lax_schema=core_schema.union_schema(
                [
                    ...
                    core_schema.no_info_after_validator_function(
                        path_validator, strict_inner_schema
                    ),
                ],
    ...
    def _deque_schema(self, items_type: Any) -> CoreSchema:
        ...
        check_instance = core_schema.json_or_python_schema(
            ...
            python_schema=core_schema.is_instance_schema(
                collections.deque, cls_repr="Deque"
            ),
        )

        lax_schema = core_schema.no_info_wrap_validator_function(
            deque_validator, list_schema
        )
    ...
    def _mapping_schema(self, tp: Any, keys_type: Any, values_type: Any) -> CoreSchema:
        ...
        if mapped_origin is dict:
            ...
            if tp is collections.defaultdict:
                default_default_factory = get_defaultdict_default_default_factory(
                    values_type
                )
                coerce_instance_wrap = partial(
                    ...
                    partial(
                        defaultdict_validator,
                        default_default_factory=default_default_factory,
                    ),
                )
            else:
                coerce_instance_wrap = partial(
                    core_schema.no_info_after_validator_function, mapped_origin
                )
    ...
    def _add_js_function(
        self, metadata_schema: CoreSchema, js_function: Callable[..., Any]
    ) -> None:
                ...
                if core_config.get("extra_fields_behavior") == "allow":
                    ...
                    for candidate_cls in cls.__mro__[:-1]:
                        ...
                        if extras_annotation is not None:
                            ...
                            extra_keys_type, extra_items_type = (
                            ...
                            if extra_keys_type is not str:
                                extras_keys_schema = self.generate_schema(
                                    extra_keys_type
                                )
                            if not typing_objects.is_any(extra_items_type):
                                extras_schema = self.generate_schema(extra_items_type)
                            if (
                                extras_keys_schema is not None
                                or extras_schema is not None
                            ):
                                break

                generic_origin: type[BaseModel] | None = getattr(
                    cls, "__pydantic_generic_metadata__", {}
                ).get("origin")

                if cls.__pydantic_root_model__:
                    root_field = self._common_field_schema(
                    ...
                    fields_schema: core_schema.CoreSchema = (
                        core_schema.model_fields_schema(
                            ...
                            computed_fields=[
                                self._computed_field_schema(
                                    d, decorators.field_serializers
                                )
                    ...
                    inner_schema = apply_validators(
                        fields_schema, decorators.root_validators.values(), None
                    )
                    inner_schema = apply_model_validators(
                        inner_schema, model_validators, "inner"
                    )
    ...
    def _generate_schema_from_get_schema_method(
        ...
        is_base_model_get_schema = (
            getattr(get_schema, "__func__", None)
            is BaseModel_.__get_pydantic_core_schema__.__func__  # pyright: ignore[reportFunctionMemberAccess]
        )
        ...
        if (
            ...
            schema = get_schema(
                ...
                CallbackGetCoreSchemaHandler(
                    self._generate_schema_inner, self, ref_mode=ref_mode
                ),
            )
            if schema["type"] == "definitions":
                schema = self.defs.unpack_definitions(schema)
        ...
        if (
            get_schema is None
            and (validators := getattr(obj, "__get_validators__", None)) is not None
        ):
            ...
            return core_schema.chain_schema(
                [
                    core_schema.with_info_plain_validator_function(v)
                    for v in validators()
                ]
            )
    ...
    def _resolve_forward_ref(self, obj: Any) -> Any:
        ...
        if isinstance(obj, ForwardRef):
            raise PydanticUndefinedAnnotation(
                obj.__forward_arg__, f"Unable to evaluate forward reference {obj}"
            )
    ...
    def _get_args_resolving_forward_refs(
        self, obj: Any, required: Literal[True]
    ) -> tuple[Any, ...]: ...
    ...
    def _get_args_resolving_forward_refs(
        ...
        if args:
            ...
            args = tuple(
                self._resolve_forward_ref(a) if isinstance(a, ForwardRef) else a
                for a in args
            )
        elif required:  # pragma: no cover
            raise TypeError(
                f"Expected {obj} to have generic parameters but it had none"
            )
        return args
        ...
        if (
            ...
            if isinstance(alias_generator, AliasGenerator):
                alias, validation_alias, serialization_alias = (
                    alias_generator.generate_aliases(field_name)
                )
    ...
    def _apply_alias_generator_to_computed_field_info(
        ...
        if (
            ...
            if isinstance(alias_generator, AliasGenerator):
                alias, validation_alias, serialization_alias = (
                    alias_generator.generate_aliases(computed_field_name)
                )
            elif isinstance(alias_generator, Callable):
    ...
    def _apply_field_title_generator_to_field_info(
        ...
        if field_info.title is None:
            ...
            if not isinstance(title, str):
                raise TypeError(
                    f"field_title_generator {field_title_generator} must return str, not {title.__class__}"
                )
    ...
    def _common_field_schema(  # C901
        ...
        def set_discriminator(schema: CoreSchema) -> CoreSchema:
            schema = self._apply_discriminator_to_union(
                schema, field_info.discriminator
            )
            return schema
        ...
        # note that this won't work for any Annotated types that get wrapped by a function validator
        # but that's okay because that didn't exist in V1
        this_field_validators = filter_field_decorator_info_by_field(
            decorators.validators.values(), name
        )
        ...
        schema = self._apply_field_serializers(
            ...
            filter_field_decorator_info_by_field(
                decorators.field_serializers.values(), name
            ),
        )
        ...
        pydantic_js_updates, pydantic_js_extra = (
            _extract_json_schema_info_from_field_info(field_info)
        )
    ...
    def _union_schema(self, union_type: Any) -> core_schema.CoreSchema:
        ...
        if len(choices) == 1:
            ...
            for choice in choices:
                tag = cast(CoreMetadata, choice.get("metadata", {})).get(
            ...
            with self._ns_resolver.push(origin):
                try:
                    annotation = _typing_extra.eval_type(
                        origin.__value__, *self._types_namespace
                    )
                except NameError as e:
                    raise PydanticUndefinedAnnotation.from_name_error(e) from e
    ...
    def _literal_schema(self, literal_type: Any) -> CoreSchema:
        ...
        expected = list(
            get_literal_values(
                literal_type, type_check=False, unpack_type_aliases="eager"
            )
        )
    ...
    def _typed_dict_schema(
        ...
        with (
            ...
            with self._config_wrapper_stack.push(config):
                core_config = self._config_wrapper.core_config(
                ...
                if self._config_wrapper.use_attribute_docstrings:
                    field_docstrings = extract_docstrings_from_cls(
                ...
                for field_name, annotation in annotations.items():
                    field_info = FieldInfo.from_annotation(
                        annotation, _source=AnnotationSource.TYPED_DICT
                    )
                    field_info.annotation = replace_types(
                        field_info.annotation, typevars_map
                    )

                    required = (
                        field_name in required_keys
                        or "required" in field_info._qualifiers
    ...
    def _namedtuple_schema(
        self, namedtuple_cls: Any, origin: Any
    ) -> core_schema.CoreSchema:
        ...
        with (
            ...
            schema = core_schema.call_schema(
                arguments_schema, namedtuple_cls, ref=namedtuple_ref
            )
            return self.defs.create_definition_reference_schema(schema)
    ...
    def _generate_parameter_schema(
        ...
        if field.alias is not None:
            ...
            alias_generator = self._config_wrapper.alias_generator
            if (
                isinstance(alias_generator, AliasGenerator)
                and alias_generator.alias is not None
            ):
    ...
    def _union_is_subclass_schema(self, union_type: Any) -> core_schema.CoreSchema:
        ...
        return core_schema.union_schema(
            [self.generate_schema(type[args]) for args in args]
        )
        ...
        if not typing_objects.is_any(items_type):
            ...
            python_schema = core_schema.chain_schema(
                [
                    python_schema,
                    core_schema.no_info_wrap_validator_function(
                        sequence_validator, list_schema
                    ),
                ],
            )
    ...
    def _dataclass_schema(
        ...
        with (
            ...
            with (
                self._ns_resolver.push(dataclass),
                self._config_wrapper_stack.push(config),
            ):
                ...
                decorators = dataclass.__dict__.get(
                    "__pydantic_decorators__"
                ) or DecoratorInfos.build(dataclass)
    ...
    def _call_schema(
        self, function: ValidateCallSupportedTypes
    ...
    def _arguments_schema(
        ...
        parameters_callback: ParametersCallback | None = None,
        ...
        for i, (name, p) in enumerate(sig.parameters.items()):
            ...
            if parameter_mode is not None:
                ...
                if unpack_type is not None:
                    ...
                    overlapping_params = non_pos_only_param_names.intersection(
                    ...
                    var_kwargs_schema = self._typed_dict_schema(
                        unpack_type, get_origin(unpack_type)
                    )
                else:
    ...
    def _arguments_v3_schema(
        ...
        mode_lookup: dict[
            _ParameterKind,
            Literal[
                "positional_only", "positional_or_keyword", "var_args", "keyword_only"
            ],
        ] = {
            Parameter.POSITIONAL_ONLY: "positional_only",
            Parameter.POSITIONAL_OR_KEYWORD: "positional_or_keyword",
    ...
    def _computed_field_schema(
        ...
        if d.info.return_type is not PydanticUndefined:
            ...
            try:
                ...
                return_type = _decorators.get_callable_return_type(
                    d.func, localns=self._types_namespace.locals
                )
            except NameError as e:
                raise PydanticUndefinedAnnotation.from_name_error(e) from e
        ...
        return_type_schema = self._apply_field_serializers(
            return_type_schema,
            filter_field_decorator_info_by_field(
                field_serializers.values(), d.cls_var_name
            ),
        )
        ...
        pydantic_js_updates, pydantic_js_extra = (
            _extract_json_schema_info_from_field_info(d.info)
        )
        core_metadata: dict[str, Any] = {}
        update_core_metadata(
            core_metadata,
    ...
    def _apply_annotations(
        ...
        """
        annotations = list(
            _known_annotated_metadata.expand_grouped_metadata(annotations)
        )
    ...
    def _apply_single_annotation_json_schema(
        ...
        if isinstance(metadata, FieldInfo):
            for field_metadata in metadata.metadata:
                schema = self._apply_single_annotation_json_schema(
    ...
    def _get_wrapped_inner_schema(
        ...
        annotation_get_schema: GetCoreSchemaFunction | None = getattr(
    ...
    def _apply_field_serializers(
        ...
        if serializers:
            ...
            if schema["type"] == "definitions":
                ...
                schema["schema"] = self._apply_field_serializers(
                    inner_schema, serializers
                )
                return schema
            elif "ref" in schema:
    ...
    def _apply_model_serializers(
        ...
        if serializers:
            ...
            if serializer.info.mode == "wrap":
                ser_schema: core_schema.SerSchema = (
                    core_schema.wrap_serializer_function_ser_schema(
                        serializer.func,
                        info_arg=info_arg,
                        return_schema=return_schema,
...
_VALIDATOR_F_MATCH: Mapping[
    ...
    Callable[
        [Callable[..., Any], core_schema.CoreSchema, str | None], core_schema.CoreSchema
    ],
] = {
    (
        "before",
    ...
    (
        "before",
        "with-info",
    ): lambda f, schema, field_name: core_schema.with_info_before_validator_function(
        f, schema, field_name=field_name
    ),
    (
        "after",
        "with-info",
    ): lambda f, schema, field_name: core_schema.with_info_after_validator_function(
        f, schema, field_name=field_name
    ),
    (
        "plain",
        "with-info",
    ): lambda f, _, field_name: core_schema.with_info_plain_validator_function(
        f, field_name=field_name
    ),
    (
        "wrap",
        "with-info",
    ): lambda f, schema, field_name: core_schema.with_info_wrap_validator_function(
        f, schema, field_name=field_name
    ),
}
...
def apply_validators(
    ...
    for validator in validators:
        ...
        schema = _VALIDATOR_F_MATCH[(validator.info.mode, val_type)](
...
def resolve_original_schema(
    schema: CoreSchema, definitions: _Definitions
) -> CoreSchema | None:
...
class _Definitions:
    ...
    def get_schema_or_ref(
        self, tp: Any, /
    ) -> Generator[tuple[str, core_schema.DefinitionReferenceSchema | None]]:
    ...
    def finalize_schema(self, schema: CoreSchema) -> CoreSchema:
        ...
        for ref, inlinable_def_ref in gather_result["collected_references"].items():
            if (
                inlinable_def_ref is not None
                and (inlining_behavior := _inlining_behavior(inlinable_def_ref))
                != "keep"
            ):
                if inlining_behavior == "inline":
                    ...
                    inlinable_def_ref.update(
                        self._resolve_definition(ref, definitions)
                    )  # pyright: ignore
        ...
        for cs in gather_result["deferred_discriminator_schemas"]:
            discriminator: str | None = cs["metadata"].pop(
                "pydantic_internal_union_discriminator", None
    ...
    def _resolve_definition(
        self, ref: str, definitions: dict[str, CoreSchema]
    ) -> CoreSchema:
        ...
        visited: set[str] = set()
        while (
            definition["type"] == "definition-ref"
            and _inlining_behavior(definition) == "inline"
        ):
