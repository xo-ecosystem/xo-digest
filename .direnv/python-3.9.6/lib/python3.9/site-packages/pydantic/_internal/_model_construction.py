...
class _ModelNamespaceDict(dict):
    ...
    def __setitem__(self, k: str, v: object) -> None:
        ...
        if (
            ...
            and isinstance(existing, PydanticDescriptorProxy)
        ):
            warnings.warn(
...
class ModelMetaclass(ABCMeta):
        ...
        **kwargs: Any,
        ...
        if bases:
            ...
            cls.__pydantic_post_init__ = (
                None
                if cls.model_post_init is BaseModel_.model_post_init
            ...
            if __pydantic_generic_metadata__:
                ...
                if (
                    ...
                    if (
                        RootModelRootType in parent_parameters
                        and RootModelRootType not in parameters
                    ):
            ...
            parent_namespace: dict[str, Any] | None = getattr(
            ...
            set_model_fields(
                cls, config_wrapper=config_wrapper, ns_resolver=ns_resolver
            )
    ...
    def __instancecheck__(self, instance: Any) -> bool:
        ...
        """
        return hasattr(
            instance, "__pydantic_decorators__"
    ...
    def __subclasscheck__(self, subclass: type[Any]) -> bool:
        ...
        """
        return hasattr(
            subclass, "__pydantic_decorators__"
        ) and super().__subclasscheck__(subclass)

    @staticmethod
    def _collect_bases_data(
        bases: tuple[type[Any], ...],
    ) -> tuple[set[str], set[str], dict[str, ModelPrivateAttr]]:
    ...
    def __pydantic_fields_complete__(self) -> bool:
        ...
        field_infos = cast(
            "dict[str, FieldInfo]", self.__pydantic_fields__
    ...
    * check for items which look like fields but are not (e.g. have no annotation) and warn.
    ...
    for var_name, value in list(namespace.items()):
        if var_name == "model_config" or var_name == "__pydantic_extra__":
            ...
            isinstance(value, all_ignored_types)
            or value.__class__.__module__ == "functools"
        ):
            ...
            if var_name not in raw_annotations or not is_classvar_annotation(
                raw_annotations[var_name]
            ):
                private_attributes[var_name] = cast(
                    ModelPrivateAttr, PrivateAttr(default=value)
                )
                del namespace[var_name]
                ...
                if frame is not None:
                    try:
                        ann_type = eval_type_backport(
                            _make_forward_ref(
                                ann_type, is_argument=False, is_class=True
                            ),
...
def set_default_hash_func(cls: type[BaseModel], bases: tuple[type[Any], ...]) -> None:
    ...
    if (
        base_hash_func in {None, object.__hash__}
        or getattr(base_hash_func, "__code__", None) == new_hash_func.__code__
    ):
...
def make_hash_func(cls: type[BaseModel]) -> Any:
    getter = (
        operator.itemgetter(*cls.__pydantic_fields__.keys())
        if cls.__pydantic_fields__
        else lambda _: 0
    )
...
def set_deprecated_descriptors(cls: type[BaseModel]) -> None:
    ...
    for field, computed_field_info in cls.__pydantic_computed_fields__.items():
        if (
            ...
            and not hasattr(
                unwrap_wrapped_function(computed_field_info.wrapped_property),
                "__deprecated__",
            )
        ):
...
class _DeprecatedFieldDescriptor:
    ...
    def __get__(
        self, obj: BaseModel | None, obj_type: type[BaseModel] | None = None
    ) -> Any:
        if obj is None:
