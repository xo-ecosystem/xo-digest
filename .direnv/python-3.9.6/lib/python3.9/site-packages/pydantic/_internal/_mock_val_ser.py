...
def set_type_adapter_mocks(adapter: TypeAdapter) -> None:
    ...
    def attempt_rebuild_fn(
        attr_fn: Callable[[TypeAdapter], T],
    ) -> Callable[[], T | None]:
        def handler() -> T | None:
            if (
                adapter.rebuild(raise_errors=False, _parent_namespace_depth=5)
                is not False
            ):
...
def set_model_mocks(
    cls: type[BaseModel], undefined_name: str = "all referenced types"
) -> None:
    ...
    def attempt_rebuild_fn(
        attr_fn: Callable[[type[BaseModel]], T],
    ) -> Callable[[], T | None]:
        def handler() -> T | None:
            if (
    ...
    cls.__pydantic_core_schema__ = (
        MockCoreSchema(  # pyright: ignore[reportAttributeAccessIssue]
            undefined_type_error_message,
            code="class-not-fully-defined",
            attempt_rebuild=attempt_rebuild_fn(lambda c: c.__pydantic_core_schema__),
        )
    )
    cls.__pydantic_validator__ = (
...
def set_dataclass_mocks(
    cls: type[PydanticDataclass], undefined_name: str = "all referenced types"
) -> None:
    ...
    def attempt_rebuild_fn(
        attr_fn: Callable[[type[PydanticDataclass]], T],
    ) -> Callable[[], T | None]:
        def handler() -> T | None:
            if (
