...
def run_process(
    ...
    * Or, using `subprocess.Popen` to run a command
    ...
    if grace_period:
        logger.debug(
            "sleeping for %s seconds before watching for changes", grace_period
        )
        sleep(grace_period)
...
def raise_keyboard_interrupt(signum: int, _frame: Any) -> None:  # pragma: no cover
    logger.warning(
        "received signal %s, raising KeyboardInterrupt", signal.Signals(signum)
    )
...
def catch_sigterm() -> None:
    ...
    """
    logger.debug(
        "registering handler for SIGTERM on watchfiles process %d", os.getpid()
    )
