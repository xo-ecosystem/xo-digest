...
class SFTPServer(BaseSFTP, SubsystemHandler):
    ...
    def __init__(
        self, channel, name, server, sftp_si=SFTPServerInterface, *args, **kwargs
    ):
        """
    ...
    def start_subsystem(self, name, transport, channel):
        self.sock = channel
        self._log(DEBUG, f"Started sftp server on channel {channel!r}")
    ...
    def _response(self, request_number, t, *args):
        ...
        for item in args:
            ...
            if isinstance(item, int64):
                ...
                raise Exception(f"unknown type for {item!r} type {type(item)!r}")
    ...
    def _send_handle_response(self, request_number, handle, folder=False):
        if not issubclass(type(handle), SFTPHandle):
            ...
            return
        handle._set_name(b(f"hx{self.next_handle:d}"))
        self.next_handle += 1
        if folder:
