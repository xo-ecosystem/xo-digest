...
class PKey:
    ...
    def from_path(path, passphrase=None):
        ...
        try:
            loaded = serialization.load_ssh_private_key(data=data, password=passphrase)
        # Then fall back to assuming legacy PEM type
        except ValueError:
            loaded = serialization.load_pem_private_key(data=data, password=passphrase)
    ...
    def _write_private_key(self, f, key, format, password=None):
        ...
        f.write(
            key.private_bytes(serialization.Encoding.PEM, format, encryption).decode()
        )
...
class PublicBlob:
    ...
    def from_string(cls, string):
        ...
        if blob_type != key_type:
            deets = f"key type={key_type!r}, but blob type={blob_type!r}"
