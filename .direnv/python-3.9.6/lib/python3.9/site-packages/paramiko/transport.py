...
class Transport(threading.Thread, ClosingContextManager):
    ...
    _PROTO_ID = "2.0"
    _CLIENT_ID = f"paramiko_{paramiko.__version__}"
    ...
    def connect(
        ...
        if (hostkey is not None) and not gss_kex:
            ...
            if (
                ...
                self._log(
                    DEBUG,
                    f"Got     : {key.get_name()}: {repr(key.asbytes())}",
                )
    ...
    def _verify_key(self, host_key, sig):
        ...
        if not key.verify_ssh_sig(self.H, Message(sig)):
            raise SSHException(
                f"Signature verification ({self.host_key_type}) failed."
            )  # noqa
        self.host_key = key
    ...
    def run(self):
        ...
        if self.server_mode:
            self._log(DEBUG, f"starting thread (server mode): {tid}")
        else:
            self._log(DEBUG, f"starting thread (client mode): {tid}")
        try:
            try:
                self.packetizer.write_all(b(self.local_version + "\r\n"))
                self._log(
                    DEBUG,
                    f"Local version/idstring: {self.local_version}",
                )  # noqa
                self._check_banner()
                ...
                while self.active:
                    ...
                    if ptype in self._handler_table:
                        ...
                        warning = f"Oops, unhandled type {ptype} ({name!r})"
    ...
    def _log_agreement(self, which, local, remote):
        ...
        if local == remote:
            ...
            msg += f"local={local}, remote={remote}"
        self._log(DEBUG, msg)
    ...
    def _check_banner(self):
        ...
        self._log(DEBUG, f"Remote version/idstring: {buf}")
    ...
    def _parse_kex_init(self, m):
        ...
        for prefix, value in (
            ...
            if value == [""]:
                value = ["<none>"]
            value = ", ".join(value)
            self._log(DEBUG, f"{prefix}: {value}")
        self._log(DEBUG, f"kex follows: {kex_follows}")
        ...
        self._log(DEBUG, f"Kex: {agreed_kex[0]}")
    ...
    def _parse_ext_info(self, msg):
        ...
        self._log(DEBUG, f"Got EXT_INFO: {extensions}")
    ...
    def _parse_disconnect(self, m):
        code = m.get_int()
        desc = m.get_text()
        self._log(INFO, f"Disconnect (code {code:d}): {desc}")

    def _parse_global_request(self, m):
        kind = m.get_text()
        self._log(DEBUG, f'Received global request "{kind}"')
        want_reply = m.get_boolean()
        if not self.server_mode:
            self._log(
                DEBUG,
                f'Rejecting "{kind}" global request from server.',
            )
            ok = False
    ...
    def _parse_channel_open_success(self, m):
        ...
        try:
            ...
            self._log(DEBUG, f"Secsh channel {chanid:d} opened.")
    ...
    def _parse_channel_open(self, m):
        ...
        if kind == "auth-agent@openssh.com" and self._forward_agent_handler is not None:
            ...
            self._log(
                DEBUG,
                f"Incoming x11 connection from {origin_addr}:{origin_port:d}",
            )
            self.lock.acquire()
            try:
                my_chanid = self._next_channel()
            ...
            self._log(
                DEBUG,
                f'Rejecting "{kind}" channel request from server.',
            )
            reject = True
            ...
            if reason != OPEN_SUCCEEDED:
                self._log(
                    DEBUG,
                    f'Rejecting "{kind}" channel request from client.',
                )
                reject = True
        if reject:
            msg = Message()
        ...
        self._send_message(m)
        self._log(DEBUG, f"Secsh channel {my_chanid:d} ({kind}) opened.")
    ...
    def _parse_debug(self, m):
        ...
        self._log(DEBUG, f"Debug msg: {util.safe_string(msg)}")
...
class SecurityOptions:
    ...
    def __repr__(self):
        ...
        Returns a string representation of this object, for debugging.
        """
        return f"<paramiko.SecurityOptions for {self._transport!r}>"
