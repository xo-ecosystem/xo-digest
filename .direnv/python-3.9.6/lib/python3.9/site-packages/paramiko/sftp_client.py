...
class SFTPClient(BaseSFTP, ClosingContextManager):
    ...
    def __init__(self, sock):
        ...
        self._log(
            INFO,
            f"Opened sftp connection (server version {server_version})",
        )
    ...
    def listdir_attr(self, path="."):
        ...
        self._log(DEBUG, f"listdir({path!r})")
    ...
    def open(self, filename, mode="r", bufsize=-1):
        ...
        self._log(DEBUG, f"open({filename!r}, {mode!r})")
        imode = 0
        ...
        self._log(
            DEBUG,
            f"open({filename!r}, {mode!r}) -> {u(hexlify(handle))}",
        )
    ...
    def remove(self, path):
        ...
        """
        path = self._adjust_cwd(path)
        self._log(DEBUG, f"remove({path!r})")
    ...
    def rename(self, oldpath, newpath):
        ...
        self._log(DEBUG, f"rename({oldpath!r}, {newpath!r})")
    ...
    def posix_rename(self, oldpath, newpath):
        ...
        oldpath = self._adjust_cwd(oldpath)
        newpath = self._adjust_cwd(newpath)
        self._log(DEBUG, f"posix_rename({oldpath!r}, {newpath!r})")
    ...
    def mkdir(self, path, mode=o777):
        ...
        """
        path = self._adjust_cwd(path)
        self._log(DEBUG, f"mkdir({path!r}, {mode!r})")
        attr = SFTPAttributes()
        attr.st_mode = mode
    ...
    def rmdir(self, path):
        ...
        """
        path = self._adjust_cwd(path)
        self._log(DEBUG, f"rmdir({path!r})")
    ...
    def lstat(self, path):
        ...
        """
        path = self._adjust_cwd(path)
        self._log(DEBUG, f"lstat({path!r})")
    ...
    def symlink(self, source, dest):
        ...
        self._log(DEBUG, f"symlink({source!r}, {dest!r})")
        source = b(source)
    ...
    def chmod(self, path, mode):
        ...
        """
        path = self._adjust_cwd(path)
        self._log(DEBUG, f"chmod({path!r}, {mode!r})")
        attr = SFTPAttributes()
        attr.st_mode = mode
    ...
    def chown(self, path, uid, gid):
        ...
        """
        path = self._adjust_cwd(path)
        self._log(DEBUG, f"chown({path!r}, {uid!r}, {gid!r})")
        attr = SFTPAttributes()
    ...
    def utime(self, path, times):
        ...
        self._log(DEBUG, f"utime({path!r}, {times!r})")
        attr = SFTPAttributes()
    ...
    def truncate(self, path, size):
        ...
        """
        path = self._adjust_cwd(path)
        self._log(DEBUG, f"truncate({path!r}, {size!r})")
        attr = SFTPAttributes()
        attr.st_size = size
        self._request(CMD_SETSTAT, path, attr)
    ...
    def readlink(self, path):
        ...
        """
        path = self._adjust_cwd(path)
        self._log(DEBUG, f"readlink({path!r})")
    ...
    def normalize(self, path):
        ...
        """
        path = self._adjust_cwd(path)
        self._log(DEBUG, f"normalize({path!r})")
    ...
    def putfo(self, fl, remotepath, file_size=0, callback=None, confirm=True):
        ...
        if confirm:
            ...
            if s.st_size != size:
                raise OSError(f"size mismatch in put!  {s.st_size} != {size}")
        else:
            s = SFTPAttributes()
        return s
    ...
    def get(
        ...
        s = os.stat(localpath)
        if s.st_size != size:
            raise OSError(f"size mismatch in get!  {s.st_size} != {size}")
    ...
    def _convert_status(self, msg):
        ...
        if code == SFTP_OK:
            ...
            raise OSError(errno.ENOENT, text)
        elif code == SFTP_PERMISSION_DENIED:
            raise OSError(errno.EACCES, text)
        else:
            raise OSError(text)
