...
class Packetizer:
    ...
    def send_message(self, data):
        ...
        try:
            ...
            if self.__dump_packets:
                self._log(
                    DEBUG,
                    f"Write packet <{cmd_name}>, length {orig_len}",
                )
            ...
            if self.__block_engine_out is not None and not self.__aead_out:
                ...
                out += compute_hmac(self.__mac_key_out, payload, self.__mac_engine_out)[
                    : self.__mac_size_out
                ]
            next_seq = (self.__sequence_number_out + 1) & xffffffff
            if next_seq == 0 and not self._initial_kex_done:
                raise SSHException("Sequence number rolled over during initial kex!")
            ...
            if sent_too_much and not self.__need_rekey:
                ...
                self._log(DEBUG, msg.format(self.__sent_packets, self.__sent_bytes))
                self.__received_bytes_overflow = 0
                self.__received_packets_overflow = 0
                self._trigger_rekey()
        finally:
    ...
    def read_message(self):
        ...
        if self.__etm_in:
            ...
            my_mac = compute_hmac(self.__mac_key_in, mac_payload, self.__mac_engine_in)[
                : self.__mac_size_in
            ]
        ...
        if self.__dump_packets:
            self._log(
                DEBUG,
                f"Got payload ({packet_size} bytes, {padding} padding)",
            )
        ...
        if self.__need_rekey:
            ...
            if (
                self.__received_packets_overflow >= self.REKEY_PACKETS_OVERFLOW_MAX
            ) or (self.__received_bytes_overflow >= self.REKEY_BYTES_OVERFLOW_MAX):
        ...
        if self.__dump_packets:
            self._log(
                DEBUG,
                f"Read packet <{cmd_name}>, length {len(payload)}",
            )
        return cmd, msg
