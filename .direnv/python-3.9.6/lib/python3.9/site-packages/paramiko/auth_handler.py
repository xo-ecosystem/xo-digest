...
class AuthHandler:
    ...
    def _generate_key_from_request(self, algorithm, keyblob):
        ...
        if algorithm.replace("-cert-v01@openssh.com", "") not in options:
            err = "Auth rejected: pubkey algorithm '{}' unsupported or disabled"
    ...
    def _finalize_pubkey_algorithm(self, key_type):
        ...
        self._log(
            DEBUG,
            f"Finalizing pubkey algorithm for key of type {key_type!r}",
        )
        ...
        if key_type.endswith("-cert-v01@openssh.com") and re.search(
            ...
            self._log(DEBUG, f"Agreed upon {pubkey_algo!r} pubkey algorithm")
            return pubkey_algo
        ...
        if server_algo_str:
            ...
            if agreement:
                ...
                self._log(
                    DEBUG,
                    f"Agreed upon {pubkey_algo!r} pubkey algorithm",
                )
            else:
...
""".format(
                    ...
                    raise SSHException(f"Received Package: {MSG_NAMES[ptype]}")
            elif self.auth_method == "gssapi-keyex" and self.transport.gss_kex_used:
    ...
    def _parse_userauth_request(self, m):
        ...
        if method == "none":
            ...
            if result != AUTH_FAILED:
                ...
                blob = self._get_session_blob(key, service, username, algorithm)
    ...
    def _parse_userauth_failure(self, m):
        ...
        if partial:
            ...
            for msg in (
                f"Authentication type ({self.auth_method}) not permitted.",
                f"Allowed methods: {authlist}",
            ):
                self._log(DEBUG, msg)
