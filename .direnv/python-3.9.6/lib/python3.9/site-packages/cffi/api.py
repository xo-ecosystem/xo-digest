...
class FFI:
    ...
    def include(self, ffi_to_include):
        ...
        if not isinstance(ffi_to_include, FFI):
            raise TypeError(
                "ffi.include() expects an argument that is also of"
                " type cffi.FFI, not %r" % (type(ffi_to_include).__name__,)
            )
    ...
    def distutils_extension(self, tmpdir="build", verbose=True):
        ...
        if source is None:
            raise TypeError(
                "distutils_extension() is only for C extension "
                "modules, not for dlopen()-style pure Python "
                "modules"
            )
        mkpath(tmpdir)
        ext, updated = recompile(
            self,
            module_name,
            source,
            tmpdir=tmpdir,
    ...
    def emit_c_code(self, filename):
        ...
        if source is None:
            raise TypeError(
                "emit_c_code() is only for C extension modules, "
    ...
    def emit_python_code(self, filename):
        ...
        if source is not None:
            raise TypeError(
                "emit_python_code() is only for dlopen()-style "
