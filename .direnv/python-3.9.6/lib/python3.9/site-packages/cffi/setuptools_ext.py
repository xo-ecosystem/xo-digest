...
def add_cffi_module(dist, mod_spec):
    ...
    try:
        ...
        error(f"{mod_spec!r} must be of the form 'path/build.py:ffi_variable'")
    if not os.path.exists(build_file_name):
        ext = ""
        rewritten = build_file_name.replace(".", "/") + ".py"
        if os.path.exists(rewritten):
            ext = " (rewrite cffi_modules to [{!r}])".format(
                rewritten + ":" + ffi_var_name
            )
        error(f"{build_file_name!r} does not name an existing file{ext}")
    ...
    try:
        ...
        error(f"{mod_spec!r}: object {ffi_var_name!r} not found in module")
    if not isinstance(ffi, FFI):
        ffi = ffi()  # maybe it's a function instead of directly an ffi
    if not isinstance(ffi, FFI):
        error(
            "{!r} is not an FFI instance (got {!r})".format(
                mod_spec, type(ffi).__name__
            )
        )
    if not hasattr(ffi, "_assigned_source"):
        error(f"{mod_spec!r}: the set_source() method was not called")
