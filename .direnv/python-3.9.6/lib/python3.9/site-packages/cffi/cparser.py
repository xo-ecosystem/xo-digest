...
_r_define = re.compile(
    r"^\s*#\s*define\s+([A-Za-z_][A-Za-z_0-9]*)" r"\b((?:[^\n\\]|\\.)*?)$",
    re.DOTALL | re.MULTILINE,
)
...
_r_extern_python = re.compile(
    r'\bextern\s*"' r'(Python|Python\s*\+\s*C|C\s*\+\s*Python)"\s*.'
)
_r_star_const_space = re.compile(  # matches "* const "
    r"[*]\s*((const|volatile|restrict)\b\s*)+"
)
_r_int_dotdotdot = re.compile(
    r"(\b(int|long|short|signed|unsigned|char)\s*)+" r"\.\.\."
)
        ...
        if csource[endpos] == "{":
            ...
            if csource.find("{", endpos + 1, closing) >= 0:
                raise NotImplementedError(
                    "cannot use { } inside a block " "'extern \"Python\" { ... }'"
                )
...
def _warn_for_string_literal(csource):
    ...
    for line in csource.splitlines():
        if '"' in line and not line.lstrip().startswith("#"):
            ...
            warnings.warn(
                "String literal found in cdef() or type source. "
...
def _warn_for_non_extern_non_static_global_variable(decl):
    if not decl.storage:
        ...
        warnings.warn(
            "Global variable '%s' in cdef(): for consistency "
    ...
    def replace(m):
        ...
        if not s.startswith("#line@"):
            raise AssertionError(
                "unexpected #line directive " "(should have been processed and removed"
            )
...
class Parser:
    ...
    def _parse(self, csource):
        ...
        csourcelines.append(
            "typedef int __dotdotdotint__, __dotdotdotfloat__," " __dotdotdot__;"
        )
    ...
    def parse(self, csource, override=False, packed=False, pack=None, dllexport=False):
        if packed:
            if packed != True:
                raise ValueError(
                    "'packed' should be False or True; use "
        ...
        try:
            ...
            for decl in iterator:
                ...
                if isinstance(decl, pycparser.c_ast.Decl):
                    ...
                    quals = 0
                    if isinstance(
                        decl.type.type, pycparser.c_ast.IdentifierType
                    ) and decl.type.type.names[-1].startswith("__dotdotdot"):
                        realtype = self._get_unknown_type(decl)
                    elif (
                        isinstance(decl.type, pycparser.c_ast.PtrDecl)
                        and isinstance(decl.type.type, pycparser.c_ast.TypeDecl)
                        and isinstance(
                            decl.type.type.type, pycparser.c_ast.IdentifierType
                        )
                        and decl.type.type.type.names[-1].startswith("__dotdotdot")
                    ):
                    ...
                    raise CDefError(
                        "unexpected <%s>: this construct is valid "
                        "C but not valid in cdef()" % decl.__class__.__name__,
                        decl,
                    )
    ...
    def _add_constants(self, key, val):
        if key in self._int_constants:
            ...
            raise FFIError(f"multiple declarations of constant: {key}")
        ...
        if isinstance(node, pycparser.c_ast.FuncDecl):
            ...
            if decl.name:
                ...
                if tp.is_raw_function:
                    ...
                    and hasattr(decl.init, "value")
                    and _r_int_literal.match(decl.init.value)
                ):
                    self._add_integer_constant(decl.name, decl.init.value)
                elif (
                    tp.is_integer_type()
                    and isinstance(decl.init, pycparser.c_ast.UnaryOp)
                    and decl.init.op == "-"
                    and hasattr(decl.init.expr, "value")
                    and _r_int_literal.match(decl.init.expr.value)
                ):
                    self._add_integer_constant(decl.name, "-" + decl.init.expr.value)
                elif tp is model.void_type and decl.name.startswith(
                    "__cffi_extern_python_"
                ):
    ...
    def _get_type_pointer(self, type, quals, declname=None):
        ...
        if (
            isinstance(type, model.StructOrUnionOrEnum)
            and type.name.startswith("$")
            and type.name[1:].isdigit()
            and type.forcename is None
            and declname is not None
        ):
            return model.NamedPointerType(type, declname, quals)
        return model.PointerType(type, quals)

    def _get_type_and_quals(
        self, typenode, name=None, partial_length_ok=False, typedef_example=None
    ):
        # first, dereference typedefs, if we have it already parsed, we're good
        if (
            isinstance(typenode, pycparser.c_ast.TypeDecl)
            and isinstance(typenode.type, pycparser.c_ast.IdentifierType)
            and len(typenode.type.names) == 1
            and ("typedef " + typenode.type.names[0]) in self._declarations
        ):
        ...
        if isinstance(typenode, pycparser.c_ast.ArrayDecl):
            ...
            #
            tp, quals = self._get_type_and_quals(
                typenode.type,
        ...
        if isinstance(typenode, pycparser.c_ast.Struct):
            return (
                self._get_struct_union_enum_type("struct", typenode, name, nested=True),
                0,
            )
        if isinstance(typenode, pycparser.c_ast.Union):
            return (
                self._get_struct_union_enum_type("union", typenode, name, nested=True),
                0,
            )
        #
    ...
    def _parse_function_type(self, typenode, funcname=None):
        ...
        for i, arg in enumerate(params):
            if not hasattr(arg, "type"):
                raise CDefError(
                    "%s arg %d: unknown type '%s'"
                    " (if you meant to use the old C syntax of giving"
                    " untyped arguments, it is not supported)"
                    % (funcname or "in expression", i + 1, getattr(arg, "name", "?"))
                )
        ellipsis = (
            len(params) > 0
            and isinstance(params[-1].type, pycparser.c_ast.TypeDecl)
            and isinstance(params[-1].type.type, pycparser.c_ast.IdentifierType)
            and params[-1].type.type.names == ["__dotdotdot__"]
        )
        if ellipsis:
            params.pop()
            if not params:
        ...
        args = [
            self._as_func_arg(*self._get_type_and_quals(argdeclnode.type))
    ...
    def _get_struct_union_enum_type(self, kind, type, name=None, nested=False):
        ...
        for decl in type.decls:
            if (
                isinstance(decl.type, pycparser.c_ast.IdentifierType)
                and "".join(decl.type.names) == "__dotdotdot__"
            ):
    ...
    def _parse_constant(self, exprnode, partial_length_ok=False):
        ...
        if isinstance(exprnode, pycparser.c_ast.Constant):
            ...
            if "0" <= s[0] <= "9":
                ...
                s[0] == "'"
                and s[-1] == "'"
                and (len(s) == 3 or (len(s) == 4 and s[1] == "\\"))
            ):
                return ord(s[-2])
            else:
                raise CDefError(f"invalid constant {s!r}")
        ...
        if (
            isinstance(exprnode, pycparser.c_ast.ID)
            and exprnode.name in self._int_constants
        ):
            return self._int_constants[exprnode.name]
        #
        if (
            isinstance(exprnode, pycparser.c_ast.ID)
            and exprnode.name == "__dotdotdotarray__"
        ):
            if partial_length_ok:
                self._partial_length = True
                return "..."
            raise FFIError(
                ":%d: unsupported '[...]' here, cannot derive "
                "the actual array length in this context" % exprnode.coord.line
            )
        #
        ...
        #
        raise FFIError(
            ":%d: unsupported expression: expected a "
