...
class BranchNode(Node):
    def __init__(self, *args, **kwargs):
        super().__init__(
            id=kwargs["id"],
            max_byte_range=kwargs["max_byte_range"],
...
class LeafNode(Node):
    def __init__(self, *args, **kwargs):
        super().__init__(max_byte_range=kwargs["max_byte_range"])
...
class Proof:
    ...
    def to_dict(self):
        return {"offset": self.offset, "proof": base64url_encode(self.proof).decode()}
...
def generate_transaction_chunks(file_handler):
    ...
    return {"data_root": root.id, "chunks": chunks, "proofs": proofs}
...
def hash_branch(left, right=None):
    ...
    return BranchNode(
        id=hash(
            [hash(left.id), hash(right.id), hash(int_to_buffer(left.max_byte_range))]
        ),
...
def hash_leaf(data, note):
    return HashNode(hash([hash(data), hash(note_to_buffer(note))]), note)
...
def validate_path(id, dest, left_bound, right_bound, path):
    ...
    if len(path) == HASH_SIZE + NOTE_SIZE:
        ...
        path_data_hash = hash([hash(path_data), hash(end_offset_buffer)])

        result = id == path_data_hash

        if result:
            return ValidatedPathResult(
                right_bound - 1, left_bound, right_bound, right_bound - left_bound
            )
    ...
    offset_buffer = path[
        left_length + right_length : left_length + right_length + NOTE_SIZE
    ]
    offset = buffer_to_int(offset_buffer)

    remainder = path[left_length + right_length + len(offset_buffer) :]

    path_hash = hash([hash(left), hash(right), hash(offset_buffer)])

    if id == path_hash:
        if dest < offset:
            return validate_path(
                left, dest, left_bound, min(right_bound, offset), remainder
            )

        return validate_path(
            right, dest, max(left_bound, offset), right_bound, remainder
        )
...
def debug(proof, output=""):
    ...
    offset_buffer = proof[len(left) + len(right) : len(left) + len(right) + HASH_SIZE]
    ...
    updated_output = "{}\n{},{},{} => {}".format(
        output, bytearray(left), bytearray(right), offset, bytearray(path_hash)
    )
