...
class TransactionUploader:
    ...
    def pct_complete(self):
        return int(f"{self.uploaded_chunks / self.total_chunks * 100}".split(".")[0])
    ...
    def upload_chunk(self):
        ...
        if self.total_errors == 100:
            raise TransactionUploaderException(
                "Unable to complete upload: {}: {}".format(
                    self.last_response_status, self.last_response_error
                )
            )
        ...
        if not chunk_ok:
            raise TransactionUploaderException(
                f"Unable to validate chunk {self.chunk_index}"
            )
        ...
        url = f"{self.transaction.api_url}/chunk"
    ...
    def post_transaction(self, chunk):
        ...
        if upload_in_body:
            url = f"{self.transaction.api_url}/tx"
            headers = {
                "Content-Type": "application/json",
                "Accept": "application/json, text/plain, */*",
            }
...
def from_transaction_id(file_handler, transaction_str, wallet, api_url=API_URL):
    ...
    if response.status_code == 200:
        ...
        raise TransactionUploaderException(
            "Tx {} not found: {} {}".format(
                tx.get("id"), response.status_code, response.text
            )
        )
    ...
    serialized = TransactionUploader(
        ...
        file_handler=file_handler,
        transaction=Transaction(
            wallet, file_handler=file_handler, transaction=response.text
        ),
    )
