...
class Wallet:
    ...
    @property
    def balance(self):
        url = f"{self.api_url}/wallet/{self.address}/balance"
...
class Transaction:
            ...
            if float(self.quantity) > 0:
                if self.target == "":
                    raise ArweaveTransactionException(
                        "Unable to send {} AR without specifying a target address".format(
                            self.quantity
                        )
                    )
    ...
    def get_reward(self, data_size, target_address=None):
        ...
        url = f"{self.api_url}/price/{data_size}"

        if target_address:
            url = f"{self.api_url}/price/{data_size}/{target_address}"
    ...
    def get_signature_data(self):
        self.reward = self.get_reward(
            self.data_size, target_address=self.target if len(self.target) > 0 else None
        )

        if int(self.data_size) > 0 and self.data_root == "" and not self.uses_uploader:
            if type(self.data) == str:
                root_hash = compute_root_hash(
                    io.BytesIO(base64url_decode(self.data.encode("utf-8")))
                )
        ...
        if self.format == 1:
            ...
            for tag in self.tags:
                ...
                tag_str += f"{name.decode()}{value.decode()}"
            ...
            signature_data = (
                owner + target + data + quantity + reward + last_tx + tag_str.encode()
            )
    ...
    def get_status(self):
        url = f"{self.api_url}/tx/{self.id}/status"
    ...
    def get_price(self):
        url = f"{self.api_url}/price/{self.data_size}"
...
def arql(wallet, query):
    ...
    headers = {"Content-type": "application/json", "Accept": "text/plain"}
    response = requests.post(f"{API_URL}/arql", data=data, headers=headers)
