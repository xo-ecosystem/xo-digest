...
class BaseParser:
    ...
    def __init__(self) -> None:
        self.logger = logging.getLogger(__name__)
        self.callbacks: (
            QuerystringCallbacks | OctetStreamCallbacks | MultipartCallbacks
        ) = {}
    ...
    def set_callback(
        self, name: CallbackName, new_func: Callable[..., Any] | None
    ) -> None:
...
class MultipartParser(BaseParser):
    ...
    def _internal_write(self, data: bytes, length: int) -> int:
        ...
        def data_callback(
            name: CallbackName, end_i: int, remaining: bool = False
        ) -> None:
            ...
            if state == MultipartState.START:
                ...
                if index == len(boundary) - 2:
                    ...
                    if c != boundary[index + 2]:
                        msg = "Expected boundary character %r, got %r at index %d" % (
                            boundary[index + 2],
                            c,
                            index + 2,
                        )
    ...
    def __init__(
        ...
        if content_type == "application/octet-stream":
            ...
            def on_start() -> None:
                nonlocal file
                file = FileClass(
                    file_name, None, config=cast("FileConfig", self.config)
                )
            ...
            def on_headers_finished() -> None:
                ...
                if file_name is None:
                    ...
                    f_multi = FileClass(
                        file_name, field_name, config=cast("FileConfig", self.config)
                    )
                    is_file = True
                ...
                if transfer_encoding in (b"binary", b"8bit", b"7bit"):
                    ...
                    if self.config["UPLOAD_ERROR_ON_BAD_CTE"]:
                        raise FormParserError(
                            'Unknown Content-Transfer-Encoding "{!r}"'.format(
                                transfer_encoding
                            )
                        )
                    else:
