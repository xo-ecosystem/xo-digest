...
if os.path.exists("/sys/devices/system/cpu/cpufreq/policy0") or os.path.exists(
    ...
    def cpu_freq():
        ...
        paths = glob.glob("/sys/devices/system/cpu/cpufreq/policy[0-9]*") or glob.glob(
            ...
            for line in f:
                ...
                try:
                    ...
                    if " " not in line:
                        ...
                        continue
                    msg = f"error while parsing {file}; malformed line {line!r}"
...
def sensors_temperatures():
    ...
    if not basenames:
        ...
        for base in basenames:
            ...
            trip_points = {
                "_".join(os.path.basename(p).split("_")[0:3]) for p in trip_paths
            }
            critical = None
            high = None
...
class Process:
    ...
    if HAS_PROC_SMAPS_ROLLUP or HAS_PROC_SMAPS:
        ...
        def _parse_smaps_rollup(self):
            ...
            uss = pss = swap = 0
            with open_binary(f"{self._procfs_path}/{self.pid}/smaps_rollup") as f:
                for line in f:
        ...
        def _parse_smaps(
            ...
            _private_re=re.compile(rb"\nPrivate.*:\s+(\d+)"),
            _pss_re=re.compile(rb"\nPss\:\s+(\d+)"),
            _swap_re=re.compile(rb"\nSwap\:\s+(\d+)"),
        ):
    ...
    @wrap_exceptions
    def num_ctx_switches(self, _ctxsw_re=re.compile(rb"ctxt_switches:\t(\d+)")):
    ...
    @wrap_exceptions
    def num_threads(self, _num_threads_re=re.compile(rb"Threads:\t(\d+)")):
    ...
    if HAS_CPU_AFFINITY:
        ...
        def _get_eligible_cpus(
            self, _re=re.compile(rb"Cpus_allowed_list:\t(\d+)-(\d+)")
        ):
    ...
    @wrap_exceptions
    def uids(self, _uids_re=re.compile(rb"Uid:\t(\d+)\t(\d+)\t(\d+)")):
        data = self._read_status_file()
        real, effective, saved = _uids_re.findall(data)[0]
        return _common.puids(int(real), int(effective), int(saved))

    @wrap_exceptions
    def gids(self, _gids_re=re.compile(rb"Gid:\t(\d+)\t(\d+)\t(\d+)")):
        data = self._read_status_file()
