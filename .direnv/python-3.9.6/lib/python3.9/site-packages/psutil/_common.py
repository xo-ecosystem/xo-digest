...
class Error(Exception):
    ...
    def __str__(self):
        ...
        if info:
            details = "({})".format(", ".join([f"{k}={v!r}" for k, v in info.items()]))
        else:
            details = None
...
def print_color(s, color=None, bold=False, file=sys.stdout):  # pragma: no cover
    ...
    if not term_supports_colors():
        ...
        try:
            ...
            msg = f"invalid color {color!r}; choose between" f" {list(colors.keys())!r}"
            raise ValueError(msg) from None
        if bold and color <= 7:
            color += 8
...
def debug(msg):
    ...
    if PSUTIL_DEBUG:
        ...
        print(f"psutil-debug [{fname}:{lineno}]> {msg}", file=sys.stderr)  # noqa: T201
