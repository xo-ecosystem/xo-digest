...
SCRIPTS_DIR = os.environ.get("PSUTIL_SCRIPTS_DIR", os.path.join(ROOT_DIR, "scripts"))
...
def _get_py_exe():
    ...
    if WINDOWS and sys.version_info >= (3, 7) and base is not None:
        ...
        exe = (
            ...
            or attempt(shutil.which("python{}.{}".format(*sys.version_info[:2])))
        ...
        for fun, name in ns.iter(ns.all, clear_cache=True):
            with self.subTest(proc=proc, name=name):
                try:
                    ...
                    msg = f"Process.{name}() didn't raise NSP and returned" f" {ret!r}"
                    raise AssertionError(msg)
...
def print_sysinfo():
    ...
    info["boot-time"] = datetime.datetime.fromtimestamp(psutil.boot_time()).strftime(
        "%Y-%m-%d %H:%M:%S"
    )
