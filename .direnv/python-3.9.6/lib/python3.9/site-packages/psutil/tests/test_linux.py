        ...
        with mock_open_exception("/proc/vmstat", FileNotFoundError) as m:
            with warnings.catch_warnings(record=True) as ws:
                ...
                assert (
                    ...
                    "be determined and were set to 0" in str(w.message)
                )
                assert ret.sin == 0
                assert ret.sout == 0
...
class TestSystemCPUCountLogical(PsutilTestCase):
    ...
    @pytest.mark.skipif(not shutil.which("nproc"), reason="nproc utility not available")
    def test_against_nproc(self):
        num = int(sh("nproc --all"))
        assert psutil.cpu_count(logical=True) == num

    @pytest.mark.skipif(not shutil.which("lscpu"), reason="lscpu utility not available")
...
class TestSystemCPUFrequency(PsutilTestCase):
    ...
    @pytest.mark.skipif(AARCH64, reason="aarch64 does not always expose frequency")
...
class TestSensorsBattery(PsutilTestCase):
    @pytest.mark.skipif(not shutil.which("acpi"), reason="acpi utility not available")
...
class TestProcess(PsutilTestCase):
    ...
    def test_open_files_fd_gone(self):
        ...
        with open(self.get_testfn(), "w"):
            # give the kernel some time to see the new file
            call_until(lambda: len(p.open_files()) != len(files))
            with mock.patch("builtins.open", side_effect=FileNotFoundError) as m:
                assert p.open_files() == []
                assert m.called
    ...
    def test_exe_mocked(self):
        with mock.patch("psutil._pslinux.readlink", side_effect=FileNotFoundError) as m:
    ...
    def test_issue_1014(self):
        ...
        with mock_open_exception(f"/proc/{os.getpid()}/smaps", FileNotFoundError) as m:
            p = psutil.Process()
            with pytest.raises(FileNotFoundError):
                p.memory_maps()
            assert m.called
    ...
    def test_issue_2418(self):
        p = psutil.Process()
        with mock_open_exception(f"/proc/{os.getpid()}/statm", FileNotFoundError):
...
class TestProcessAgainstStatus(PsutilTestCase):
    ...
    def read_status_file(self, linestart):
        with psutil._psplatform.open_text(f"/proc/{self.proc.pid}/status") as f:
            for line in f:
                line = line.strip()
