...
def get_total():
    ...
    if hasattr(coverage.results.Numbers, "set_precision"):  # Coverage <= 5
        ...
        return coverage.results.Numbers(precision=cov.config.precision).display_covered(
            total
        )
    else:  # Coverage >= 7.5
...
def parse_args(argv=None):
    ...
    parser.add_argument(
        "-v", dest="print_version", action="store_true", help="Show version."
    )
...
def main(argv=None):
    ...
    if args.print_version:
        print(f"coverage-badge v{__version__}")
        sys.exit(0)
    ...
    try:
        ...
        print(f"Error: {e} Did you run coverage first?")
        sys.exit(1)
