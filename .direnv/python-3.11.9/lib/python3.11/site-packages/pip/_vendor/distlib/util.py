    ...
    for k, v in exports.items():
        ...
        for entry in v.values():
            if entry.suffix is None:
                ...
                s = f"{entry.prefix}:{entry.suffix}"
            if entry.flags:
                s = "{} [{}]".format(s, ", ".join(entry.flags))
...
class Sequencer:
    ...
    def remove(self, pred, succ):
        ...
        try:
            ...
            raise ValueError(f"{succ!r} not a successor of {pred!r}")
...
class CSVBase:
    defaults = {
        "delimiter": ",",  # The strs are used because we need native
        "quotechar": '"',  # str in the csv API (2.x won't take
        "lineterminator": "\n",  # Unicode)
    }
