...
SHEBANG_DETAIL_RE = re.compile(rb'^(\s*#!("[^"]+"|\S+))\s+(.*)$')
...
class Wheel:
    ...
    def filename(self):
        ...
        return "{}-{}{}-{}-{}-{}.whl".format(
            self.name, version, buildver, pyver, abi, arch
        )
    ...
    def build(self, paths, tags=None, wheel_version=None):
        ...
        for pyver, abi, arch in self.tags:
            wheel_metadata.append(f"Tag: {pyver}-{abi}-{arch}")
    ...
    def _get_dylib_cache(self):
        ...
        if cache is None:
            ...
            base = os.path.join(
                get_cache_base(), "dylib-cache", "%s.%s" % sys.version_info[:2]
            )
            cache = Cache(base)
        return cache
    ...
    def update(self, modifier, dest_dir=None, **kwargs):
        ...
        def get_version(path_map, info_dir):
            version = path = None
            key = f"{info_dir}/{LEGACY_METADATA_FILENAME}"
        ...
        def update_version(version, path):
            ...
            try:
                ...
                if i < 0:
                    ...
                    parts[-1] += 1
                    updated = "{}+{}".format(
                        version[:i], ".".join(str(i) for i in parts)
                    )
            except UnsupportedVersionError:
                logger.debug(
                    "Cannot update non-compliant (PEP-440) " "version %r", version
                )
            if updated:
        ...
        if m:
            ...
            while minor >= 0:
                for match in matches:
                    s = f"{name}_{major}_{minor}_{match}"
                ...
                if len(parts) == 2:
                    ...
                    result.append(
                        (
                            "".join((IMP_PREFIX, versions[0])),
                            abi,
                            f"manylinux_{parts[0]}_{parts[1]}_{arch}",
                        )
                    )
