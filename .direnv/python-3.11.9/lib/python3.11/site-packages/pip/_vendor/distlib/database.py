...
class DistributionPath:
    ...
    def provides_distribution(self, name, version=None):
        ...
        if version is not None:
            try:
                matcher = self._scheme.matcher(f"{name} ({version})")
            except ValueError:
                raise DistlibException(
                    f"invalid name or version: {name!r}, {version!r}"
                )
...
class Distribution:
    ...
    def __repr__(self):
        ...
        if self.source_url:
            ...
            suffix = ""
        return f"<Distribution {self.name} ({self.version}){suffix}>"
...
class InstalledDistribution(BaseInstalledDistribution):
    ...
    def __init__(self, path, metadata=None, env=None):
        ...
        if env and env._cache_enabled and path in env._cache.path:
            ...
            if r is None:
                raise ValueError(f"no {METADATA_FILENAME} found in {path}")
            with contextlib.closing(r.as_stream()) as stream:
                metadata = Metadata(fileobj=stream, scheme="legacy")

        super().__init__(metadata, path, env)
    ...
    def __repr__(self):
        return "<InstalledDistribution {!r} {} at {!r}>".format(
            self.name,
            self.version,
            self.path,
        )
...
class EggInfoDistribution(BaseInstalledDistribution):
    ...
    def __repr__(self):
        return "<EggInfoDistribution {!r} {} at {!r}>".format(
            self.name,
            self.version,
            self.path,
        )
...
class DependencyGraph:
        ...
        for dist, adjs in self.adjacency_list.items():
            ...
            for other, label in adjs:
                if label is not None:
                    ...
                    f.write(f'"{dist.name}" -> "{other.name}"\n')
    ...
    def topological_sort(self):
        ...
        while True:
            ...
            logger.debug(
                "Moving to result: %s",
                [f"{d.name} ({d.version})" for d in to_remove],
            )
...
def get_required_dists(dists, dist):
    ...
    seen = {t[0] for t in todo}  # already added to todo
