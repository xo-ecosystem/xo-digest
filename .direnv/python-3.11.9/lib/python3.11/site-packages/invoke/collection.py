...
class Collection:
    ...
    def from_module(
        ...
        def instantiate(obj_name: Optional[str] = None) -> "Collection":
            ...
            kwargs = dict(loaded_from=loaded_from, auto_dash_names=auto_dash_names)
    ...
    def task_with_config(self, name: Optional[str]) -> Tuple[str, Dict[str, Any]]:
        """
        ...
        for primary, aliases in self.task_names.items():
            ...
            result.append(
                ParserContext(
                    name=primary,
                    aliases=aliases,
                    args=task.get_arguments(ignore_unknown_help=ignore_unknown_help),
                )
            )
        return result

    def subtask_name(self, collection_name: str, task_name: str) -> str:
        return ".".join([self.transform(collection_name), self.transform(task_name)])
    ...
    def task_names(self) -> Dict[str, List[str]]:
        ...
        for coll_name, coll in self.collections.items():
            for task_name, aliases in coll.task_names.items():
                aliases = list(map(lambda x: self.subtask_name(coll_name, x), aliases))
    ...
    def serialized(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "collections": [
                x.serialized()
                for x in sorted(self.collections.values(), key=lambda x: x.name or "")
            ],
        }
