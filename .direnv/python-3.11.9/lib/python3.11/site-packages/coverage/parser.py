...
class PythonParser:
    ...
    def lines_matching(self, regex: str) -> set[TLineNo]:
        ...
        for match in re.finditer(regex, self.text, flags=re.MULTILINE):
            ...
            matches.update(
                self._multiline.get(i, i) for i in range(start_line + 1, end_line + 2)
            )
            last_start = start
    ...
    def _raw_parse(self) -> None:
        ...
        for toktype, ttext, (slineno, _), (elineno, _), ltext in tokgen:
            ...
            if toktype == token.INDENT:
                ...
                if ttext == ":" and nesting == 0:
                    should_exclude = self.excluded.intersection(
                        range(first_line, elineno + 1)
                    )
        ...
        # module.
        if (
            env.PYBEHAVIOR.module_firstline_1
            and self._multiline
            and self.raw_statements
        ):
            ...
            if isinstance(node, (ast.ClassDef, ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                if self.excluded.intersection(range(first_line, node.lineno + 1)):
                    self.excluded.update(
                        range(first_line, cast(int, node.end_lineno) + 1)
                    )
    ...
    def translate_arcs(self, arcs: Iterable[TArc]) -> set[TArc]:
        ...
        return {
            ...
            for (a, b) in self.fix_with_jumps(arcs)
        }
    ...
    def _analyze_ast(self) -> None:
        ...
        assert self._ast_root is not None
        aaa = AstArcAnalyzer(
            self.filename, self._ast_root, self.raw_statements, self._multiline
        )
        aaa.analyze()
        arcs = aaa.arcs
...
class TryBlock(Block):
    ...
    def __init__(
        self, handler_start: TLineNo | None, final_start: TLineNo | None
    ) -> None:
...
class AstArcAnalyzer:
    ...
    def add_arc(
        ...
        if self.debug:  # pragma: debugging
            print(
                f"Adding possible arc: ({start}, {end}): {missing_cause_msg!r}, {action_msg!r}"
            )
        ...
        if missing_cause_msg is not None or action_msg is not None:
            self.missing_arc_fragments[(start, end)].append(
                (missing_cause_msg, action_msg)
            )
    ...
    def _handle__If(self, node: ast.If) -> set[ArcStart]:
        ...
        if not constant_test or not val:
            from_start = ArcStart(
                start, cause="the condition on line {lineno} was always true"
            )
    ...
    if sys.version_info >= (3, 10):
        ...
        def _handle__Match(self, node: ast.Match) -> set[ArcStart]:
            ...
            if not had_wildcard:
                exits.add(
                    ArcStart(
                        case_start, cause="the pattern on line {lineno} always matched"
                    ),
                )
            return exits
    ...
    def _handle__Try(self, node: ast.Try) -> set[ArcStart]:
        ...
        if node.handlers:
            for handler_node in node.handlers:
                ...
                handler_exits |= self.process_body(
