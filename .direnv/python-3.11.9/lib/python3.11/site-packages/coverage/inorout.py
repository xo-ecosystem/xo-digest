...
class InOrOut:
    ...
    def __init__(
            ...
            for pkg in self.source_pkgs:
                ...
                if modfile:
                    if self.third_match.match(modfile):
                        ...
                        self.source_in_third_paths.add(
                            canonical_path(source_for_file(modfile))
                        )
                else:
                    for pathdir in path:
        ...
        self.source_in_third_match = TreeMatcher(
            self.source_in_third_paths, "source_in_third"
        )
    ...
    def should_trace(
        self, filename: str, frame: FrameType | None = None
    ) -> TFileDisposition:
    ...
    def check_include_omit_etc(
        self, filename: str, frame: FrameType | None
    ) -> str | None:
        ...
        if self.source_match or self.source_pkgs_match:
            ...
            if self.source_pkgs_match:
                if isinstance(modulename, str) and self.source_pkgs_match.match(
                    modulename
                ):
                    ok = True
            ...
            if self.third_match.match(
                filename
            ) and not self.source_in_third_match.match(filename):
